...
const InterviewSystem = () => {
  ...
  const topics = [
    {
      ...
      description: 'How do you organize your engineering documents and folders?'
    },
    {
      id: 'vendor_folders',
      title: '🏢 Vendor Documentation',
      description: 'Best practices for organizing vendor-specific documents'
    },
    {
      id: 'equipment_docs',
      title: '⚙️ Equipment Documentation',
      description: 'Organizing manuals, specs, and equipment-related files'
    },
    {
      id: 'procedures',
      title: '📋 Procedures & SOPs',
      description: 'How to structure standard operating procedures'
    }
  ];
  ...
  const generateFileStructure = async () => {
    try {
      ...
      if (data.response) {
        structureResponse = data.response;
      } else if (data.body && typeof data.body === 'string') {
        const parsedBody = JSON.parse(data.body);
        structureResponse = parsedBody.response;
      } else if (data.body && data.body.response) {
        structureResponse = data.body.response;
      } else {
        structureResponse = 'Error generating file structure';
      }

      setFileStructure(structureResponse);
      setShowStructure(true);

    } catch (error) {
      console.error('Error generating file structure:', error);
      setFileStructure('Sorry, there was an error generating the file structure. Please try again.');
      setShowStructure(true);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="interview-system">
      <div className="header">
        <h1>🎯 Engineering File Organization Interview</h1>
        <p>Help us understand your preferences to build better file organization systems!</p>
      </div>
      ...
      {interviewActive && (
        <div className="interview-container">
          <div className="interview-header">
            <h3>📋 Interview in Progress: {topics.find(t => t.id === currentTopic)?.title}</h3>
          ...
          <div className="input-container">
            ...
            <button 
              ...
              disabled={loading || !inputText.trim() || interviewComplete}
          ...
          {interviewComplete && (
            <div className="interview-complete">
              ...
              <p>Thank you for sharing your insights. Would you like to see a recommended file structure?</p>
              <button 
                onClick={generateFileStructure}
                disabled={loading}
                className="generate-button"
              >
                {loading ? 'Generating...' : '📁 Generate File Structure'}
              </button>
            </div>
          )}
        </div>
      ...
      {showStructure && (
        <div className="structure-container">
          ...
          <div className="structure-content">
            {fileStructure.split('\n').map((line, index) => (
              <div key={index} className="structure-line">
                {line}
              </div>
            ))}
          </div>
